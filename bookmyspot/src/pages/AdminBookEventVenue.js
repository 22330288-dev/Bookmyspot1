...
export default function AdminBookEventVenue() {
    ...
    const fetchVenueDetails = async () => {
      try {
        ...
        setVenue({
          ...
          address: `${response.data?.area || ""}, ${
            response.data?.city || ""
          }`,
        });
      } catch (error) {
        console.error("Failed to fetch event venue details:", error);
      } finally {
  ...
  useEffect(() => {
    ...
    const fetchSections = async () => {
      try {
        ...
        console.error("Failed to fetch event venue sections:", error);
        setSections([]);
        setSectionId("");
        setSectionName("");
      }
  ...
  useEffect(() => {
    ...
    const fetchMapLayout = async () => {
      try {
        ...
        console.error("Failed to fetch event venue map layout:", error);
        setMapLayout([]);
      } finally {
        setLoadingMap(false);
      }
  ...
  const mapItems = useMemo(() => {
    ...
    return mapLayout.map((item, index) => {
      ...
      return {
        ...
        isTable: ["round-table", "rect-table", "square-table"].includes(type),
      };
    });
  }, [mapLayout]);
  ...
  const renderMapItemContent = (item) => {
    switch (item.type) {
      ...
      case "rect-table":
      case "square-table":
        return item.label || item.seats || "";

      case "chair":
        return item.label || "Chair";

      case "sofa":
        return item.label || "Sofa";
  ...
  if (!venue && loadingVenue) {
    return <div className="book-page">Loading event venue...</div>;
  }

  if (!venue) {
    return <div className="book-page">Event venue not found.</div>;
  }
    ...
    <div className="book-page">
      <div className="book-container">
        <div className="book-top-image">
          <img
            src={venue.image || "/images/events/default-event.jpg"}
            alt={venue.name}
            onError={(e) => {
        ...
        <div className="book-info-card">
          ...
          <p className="book-type">
            Event Venue •{" "}
            {venue.event_type || venue.type || venue.description || "Event"}
          </p>
          ...
          <div className="selected-info">
            {selectedTable ? (
              <>
                <p>
                  Selected Table:{" "}
                  <strong>{selectedTable.label || selectedTable.type}</strong>
                </p>
