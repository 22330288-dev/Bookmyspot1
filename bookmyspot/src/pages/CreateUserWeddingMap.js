...
export default function CreateUserWeddingMap() {
  ...
  const fixedTypes = ["window", "door", "pool", "plant", "dance-floor"];
  ...
  const [items, setItems] = useState(
    Array.isArray(existingMap)
      ? existingMap.map((item, index) => ({
          ...
          label: item.label || "",
          locked: fixedTypes.includes(item.type),
        }))
      : []
  );
  ...
  const createChairsAroundTable = (table, seats) => {
    ...
    for (let i = 0; i < seats; i += 1) {
      ...
      const guestName = prompt(
        `Enter guest name for chair ${i + 1} around ${table.label || "this table"}:`
      );
  ...
  const addTable = (type) => {
    ...
    const tableLabel = prompt(
      "Enter table label/name. Example: Family Table, Friends Table, Maazem Table:"
    );

    const size = getItemSize(type);
    const tableId = `user-table-${Date.now()}-${Math.random()}`;
  ...
  const addItem = (type) => {
    ...
    } else if (type === "bride-groom-stage") {
      label = "Bride & Groom";
    }

    const item = {
      id: `user-item-${Date.now()}-${Math.random()}`,
      type,
      x: 200,
      y: 200,
      width: size.width,
    ...
    >
      <div className="create-seats-header">
        ...
        <div className="create-seats-title-wrap">
          <h1>Create Your Own Wedding Map</h1>
          <p>
          ...
          >
            {items.length === 0 && (
              <div className="empty-layout-message">
                ...
                <p>Please ask the admin to create the wedding hall map first.</p>
              </div>
            )}
