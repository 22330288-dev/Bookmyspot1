...
export default function CreateEventMap() {
  ...
  const normalizeItem = useCallback((item, index = 0) => {
    ...
    return {
      id: item.id || `event-temp-${Date.now()}-${index}`,
      type,
      item_type: type,
      ...
      seats_count: Number(item.seats_count ?? item.seats ?? 0),
      parentTableId: item.parentTableId || item.parent_table_id || null,
      parent_table_id: item.parent_table_id || item.parentTableId || null,
      isReservable:
  ...
  const fetchMapItems = useCallback(async () => {
    ...
    try {
      ...
      alert("Failed to load event venue map");
    } finally {
      setLoading(false);
    }
  ...
  const addEventTable = (type) => {
    ...
    const size = getItemSize(type);
    const tableId = `event-table-${Date.now()}-${Math.random()}`;
  ...
  const addItem = (type) => {
    ...
    const newItem = {
      id: `event-item-${Date.now()}-${Math.random()}`,
      type,
      item_type: type,
  ...
  const updateSelectedItem = (field, value) => {
    setItems((prev) =>
      prev.map((item) => {
        ...
        if (field === "seats") {
          updated.seats = Number(value);
          updated.seats_count = Number(value);
        }

        if (field === "parentTableId") {
          updated.parentTableId = value || null;
          updated.parent_table_id = value || null;
        }
  ...
  const handleSaveLayout = async () => {
    if (!venue?.id || !sectionId) {
      alert("Missing event venue or section.");
      return;
    }
    ...
    try {
      ...
      await axios.post(`${API_URL}/layout/${sectionId}`, {
        map: items,
      });

      alert("Event venue map saved successfully");
      fetchMapItems();
    } catch (error) {
      console.error("Save event map error:", error);
      alert("Failed to save event venue map");
    } finally {
      setSaving(false);
    }
  };
    ...
    >
      ...
      <div className="designer-layout-wrapper">
        <div className="designer-side-panel">
          ...
          <button type="button" onClick={() => addEventTable("round-table")}>
            <Circle size={18} />
            <span>Round Table</span>
          </button>

          <button type="button" onClick={() => addEventTable("rect-table")}>
            <Square size={18} />
          ...
          >
            {loading && (
              <div className="empty-layout-message">
                <h2>Loading...</h2>
                <p>Please wait while the event venue layout is loading.</p>
              </div>
            )}

            {!loading && items.length === 0 && (
              <div className="empty-layout-message">
                <h2>Start Designing Event Venue</h2>
                <p>
                  Add tables, chairs, stage, dance floor, pool, doors,
                  windows, and plants.
                </p>
              </div>
            )}
